/**
 * /api/catalogs — admin CRUD for digital catalogs
 *   POST   { ...full catalog record }  -> upsert
 *   DELETE ?id=                        -> delete
 *
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      await upsertRecord('catalogs', recordToUpsert(record));
    ...
    if (method === 'DELETE') {
      ...
      await deleteRecord('catalogs', id);
