...
// Note: the "مدیریت محتوا" (CMS) button was removed from the public header.
// Content management lives in the separate authenticated app at /admin.
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-[#07080d]/80 backdrop-blur-xl border-b border-white/10 transition-all">
      <div className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 lg:h-[70px] flex items-center justify-between gap-2 sm:gap-4">
        
        {/* Left: Brand */}
        <button 
          onClick={() => setActiveTab('overview')}
          className="flex items-center group focus:outline-none cursor-pointer text-right shrink-0 h-full"
          title="صفحه نخست"
        >
          <svg className="h-7 sm:h-8 lg:h-9 w-auto shrink-0 group-hover:scale-105 transition-transform" viewBox="0 0 125 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clipPath="url(#clip0_12996_1811_header)">
            ...
            <defs>
              <clipPath id="clip0_12996_1811_header">
        ...
        {/* Center: Navigation — no background container */}
        <nav className="flex items-center gap-1 overflow-x-auto scrollbar-none min-w-0 h-full">
          
          <button
            onClick={() => setActiveTab('overview')}
            className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 lg:px-5 h-full rounded-none text-[11px] sm:text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 border-b-2 ${
              activeTab === 'overview'
          ...
          <a
            href="#top10"
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 lg:px-5 h-full rounded-none text-[11px] sm:text-xs font-bold text-slate-400 hover:text-white transition-all cursor-pointer whitespace-nowrap shrink-0 border-b-2 border-transparent"
            title="پروژه‌های شاخص"
          >
            <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            <span>۱۰ پروژه شاخص</span>
          </a>

          <a
            href="#pictograms"
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 lg:px-5 h-full rounded-none text-[11px] sm:text-xs font-bold text-slate-400 hover:text-cyan-400 transition-all cursor-pointer whitespace-nowrap shrink-0 border-b-2 border-transparent"
            title="پیکتوگرام‌های برندها"
          >
          ...
          <a
            href="#catalogs"
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 lg:px-5 h-full rounded-none text-[11px] sm:text-xs font-bold text-slate-400 hover:text-emerald-400 transition-all cursor-pointer whitespace-nowrap shrink-0 border-b-2 border-transparent"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 h-full rounded-none text-[11px] sm:text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 border-b-2 ${
              activeTab === 'archive'
                ? 'text-white border-white/40'
                : 'text-slate-400 hover:text-slate-200 border-transparent'
            }`}
          >
            <FolderArchive className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            <span>آرشیو</span>
            <span className="text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded-md bg-white/10 text-slate-300 font-mono">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 h-full">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 sm:px-3 h-8 sm:h-9 rounded-lg hover:bg-white/10 text-slate-300 hover:text-white transition-all cursor-pointer"
            title="جستجوی پروژه‌ها"
            aria-label="جستجو"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 h-8 sm:h-9 rounded-lg bg-[#0066FF] hover:bg-[#0052cc] text-white font-bold text-[11px] sm:text-xs transition-all cursor-pointer hover:scale-[1.02]"
            title="ارائه تمام‌صفحه"
          >
            <MonitorPlay className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white shrink-0" />
            <span>ارائه</span>
          </button>
